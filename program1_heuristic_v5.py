...
Output  : results_heuristic_v5.csv

Vehicle fleet (4 types × 3 units = 12 vehicles):
  l=1  Blind Van    W=830kg   V=2.0CBM  e=13.5km/L  fuel=Gasoline  P_BBM=Rp10,000/L
  l=2  Pickup Bak   W=1250kg  V=5.0CBM  e=12.0km/L  fuel=Gasoline  P_BBM=Rp10,000/L
  l=3  Engkel (CDE) W=2250kg  V=9.0CBM  e=6.0km/L   fuel=Diesel    P_BBM=Rp6,800/L
  l=4  CDD Box      W=4500kg  V=15.0CBM e=4.5km/L   fuel=Diesel    P_BBM=Rp6,800/L
...
OUTPUT_PATH  = os.path.join(os.path.dirname(__file__), "results_heuristic_v5.csv")
...
def travel_cost(dist_km, vehicle_type_l):
    """Fuel cost in Rp: uses per-vehicle fuel price (gasoline vs diesel)."""
    ...
    for k in range(1, max_k + 1):
        ...
        for slot in active_slots:
            ...
            routes.append({
                ...
                "route_full":          depot_name + " -> " + " -> ".join(all_nodes[i]["location_name"] for i in route_order) + " -> " + depot_name,
...
def main():
    print("=" * 65)
    print("  HCVRP Heuristic Solver v5")
