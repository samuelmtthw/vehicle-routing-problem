...
Time limit: 60 seconds per scenario (adaptive based on n)
"""
...
OUTPUT_PATH  = os.path.join(os.path.dirname(__file__), "results_ilp_v3.csv")
...
def build_distance_matrix(nodes):
    """nodes: list of dicts with lat/lng. Returns n×n km matrix."""
    n = len(nodes)
...
# ── OR-Tools model ─────────────────────────────────────────────────────────────
def solve_scenario(scenario_id, depot, delivery_nodes, time_limit_sec=60):
    """
    ...
    The OR-Tools routing model maps naturally to the ILP:
      ...
      - Distance callbacks implement c_ijl · x_ijkl
      - AddDimensionWithVehicleCapacity implements C4 (weight) and C5 (volume)
      - The solver's flow conservation handles C1, C2, C3, C3b
      - MTZ subtour elimination (C6) is handled implicitly by OR-Tools

    Returns result dict or None if infeasible/timeout.
    """
    t0 = time.time()
    ...
    # ── Search parameters ──────────────────────────────────────────────────────
    # Adaptive time limit: more time for larger n
    adaptive_limit = min(time_limit_sec, max(30, n * 6))
    ...
    search_params.time_limit.seconds = adaptive_limit
    ...
    return {
        ...
        "runtime_sec": round(runtime, 4),
        "time_limit_used_sec": adaptive_limit,
...
def main():
    ...
    for scenario_id in sorted(scenarios.keys()):
        ...
        n  = sc["n_nodes"]
        adaptive_limit = min(60, max(30, n * 6))
        print(f"  Solving {scenario_id}  (n={n}, limit={adaptive_limit}s) ...",
              end=" ", flush=True)
